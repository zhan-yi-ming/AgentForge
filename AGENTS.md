...
- 每次交付前由 Pi Agent 运行与改动相称的测试；Codex 不执行测试，并把 Pi 报告中的命令、结果和清理状态写入本次变更记录。
...
- Pi 每一轮必须在一次报告中尽可能完整列出该交付范围内全部可确认问题（最多十项、按严重度排序），不得先反馈少量问题、等待修复后再补报同范围内本可发现的问题。Codex 收到报告后统一研判和修改，再交回 Pi 统一复验。
...
- **Codex (GPT)**：主开发 Agent。负责架构、文档、编码、修复及阶段 Git 提交与推送，不执行测试命令。Codex 可依据 Pi 建议编写或修改测试代码，但必须交由 Pi 执行。完成交付后中断，并读取审查与测试报告决定是否修复。
- **Pi Agent (DeepSeek V4-pro)**：独立代码审查员与测试执行者。模型固定为 `deepseek/deepseek-v4-pro`，禁止使用或降级到 Flash。代码审查模式完全只读；验证模式可使用受限 PowerShell 执行 Prompt 明确列出的测试，并清理由本次测试明确创建的数据，但严禁编辑业务代码、文档或 Git 状态。重点审查 Bug、权限绕过、API 契约不一致、并发/幂等问题与测试缺失，记录命令、退出码、测试数量、跳过项与清理结果。报告输出至 `docs/08-reviews/`。
