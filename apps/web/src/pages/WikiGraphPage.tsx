...
type Props = { projectId: string; pages: WikiPage[]; onBack: () => void; onOpenPage: (page: WikiPage) => void };
...
function graphFor(projectId: string, allPages: WikiPage[]) {
  ...
  sorted.forEach((page, index) => {
    if (index === 0) { positions.set(page.id, { page, x: 500, y: 320, degree: degrees.get(page.id) ?? 0 }); return; }
    const ring = Math.ceil(Math.sqrt(index));
    const previous = (ring - 1) * (ring - 1);
    const count = ring * ring - previous;
    const angle = ((index - previous) / count) * Math.PI * 2 - Math.PI / 2;
    const radiusX = Math.min(410, 165 + (ring - 1) * 110);
    const radiusY = Math.min(270, 118 + (ring - 1) * 75);
...
export default function WikiGraphPage({ projectId, pages: allPages, onBack, onOpenPage }: Props) {
  ...
  const selected = graph.pages.find((page) => page.id === selectedId);
  ...
  return <section className="wiki-graph-page" aria-label="Wiki 知识图谱">
    <header className="wiki-graph-header">
      <div><button className="wiki-graph-back" onClick={onBack}>← 返回 Wiki</button><p className="wiki-graph-kicker">AGENTFORGE / KNOWLEDGE ATLAS</p><h1>知识关系图谱<span>✦</span></h1><p>沿着真实的 Wiki 链接，探索项目知识的脉络。</p></div>
