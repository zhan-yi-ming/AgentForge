<#
.SYNOPSIS
    使用 DeepSeek V4.1 Flash 对一个 Git 交付范围执行有超时保护的只读审查。
.DESCRIPTION
...
param(
    ...
    [string[]]$ContextFiles = @(),
    [ValidateSet("deepseek/deepseek-flash")]
...
$ModelCatalog = & $PiCmd --list-models "deepseek-flash" 2>&1
if ($LASTEXITCODE -ne 0 -or (($ModelCatalog -join "`n") -notmatch '(?m)^deepseek\s+deepseek-flash\s')) {
    throw "Pi 模型目录中未找到 deepseek/deepseek-flash；审查已停止，禁止自动切换模型。"
}
