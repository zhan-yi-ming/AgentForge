...
P3-05 语音采集与降采样代码按需由独立 `/chat` 页面加载；`App.tsx` 只保持可编辑聊天草稿与既有会话路由。音频块和实时转写通过 typed API client 与 Java 通信，不把供应商密钥或地址下发浏览器；离开页面及项目切换时中止采集和轮询。语音转写不会自动触发发送或 Action。
...
- 工作区采用顶部左侧无框 Logo、右侧独立的产品说明、版本状态和退出操作，配合左侧悬浮入口与居中单列内容；项目空间与执行任务通过抽屉展开，不占用固定侧栏。首页使用紧凑多行 Composer；打开 Wiki、Task 或整理工具时 Composer 默认收为单行，放大/缩小由发送按钮左侧的独立按钮控制且不阻断输入聚焦。只有发送消息后才进入独立会话布局，此时消息区域随页面外层滚动、Composer 固定在视口底部。会话内的项目、任务与整理入口保留悬浮窗；Wiki 入口在 P3-01 后进入独立 `/wiki` 路由，左上角返回按钮回到首页。首页存在当前会话时提供重新进入入口，并在离开期间收到完整回答后显示未读角标。
...
## V3 前置路由（P3-01，Implemented）

首阶段 App 协调现有认证、项目和 API 状态，但聊天与 Wiki 视图已分离为按需加载的页面组件；既有 Task 与 AI 整理面板也使用独立按需加载视图。后续新增页面必须有独立路由和组件，继续把页面专属状态下沉，以按需加载边界控制首屏代码量。离开聊天或切换 project 时中止在途流，收到旧响应不得更新当前路由。路径仅编码页面与会话 ID，不能包含密码、Token 或 userId。

P3-03 将聊天页专属的引用开合和确认弹层可见状态保存在聊天组件中；应用壳仍只持有业务会话、待决 Action 和 API 回调。确认弹层以独立 portal 覆盖内容，关闭只更改界面状态，Core API 继续是 Action 决策权威。

P3-04 的流式来源在 `complete` 才结算；Web typed client 用最终 sources 覆盖 metadata 候选，不自行推断模型用了哪些文档。Action Intent 的生成属于 Python，Java 的校验/审批/写入边界不变。

## P3-07 Wiki 图谱路由

`/wiki/graph` 由单独的懒加载页面组件承载图布局与交互，App 仅传入当前项目 Wiki 列表、返回和打开页面回调。链接解析在 Web 中对已获授权的当前项目页面只读执行，不改变 Core API 契约或 Java 权限边界。图谱在自身页面使用独立暗色舞台，样式局限于页面专属 CSS，避免改写 V2 登录和工作台全局视觉。图谱以显式 Wiki 链接为来源，不建立 V3 图领域模型。

本地验收修正后，`/wiki/graph` 在认证后的路由分支直接挂载独立懒加载页面，不继承居中工作台容器；页面自有全视口样式、布局和交互，App 只传项目页面数据与返回/打开回调。聊天输入框内的会话及语音快捷操作仍由 Chat 页提供回调和录音状态，不把录音实现塞进 App。登录布局采用视口尺寸约束和低高度兜底，保持原有双栏视觉语言。
