...
- 工作区采用顶部独立悬浮产品栏、左侧悬浮入口和居中单列内容；项目空间与执行任务通过抽屉展开，不占用固定侧栏。Agent Chat 常驻首位，Wiki、Task、整理工具共享底部工作台并通过导航切换，切换时 Chat 收起为一行。
- 首次登录后显示 DOM 可访问的模块化引导，模块说明使用可展开内容，关闭时只在 `localStorage` 保存完成标记；顶栏可重新打开，引导不改变项目、对话或业务状态。
- Wiki 新建可选择空白页面或进入 AI 整理；整理结果应用到草稿后自动回到 Wiki 工作台。Wiki 实时预览以底部可关闭、可调整高度的悬浮窗呈现。
