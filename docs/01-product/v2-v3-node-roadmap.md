...
## V3 前置体验修复（2026-09-18 用户授权）

这些是 V2 收口后的独立修复阶段，依次排在 V3-01 之前；阶段分别记录、验证、审核、提交和远端核验。用户已明确授权按此顺序开发。V3 Node 顺序与 Scope 保持下文原样。

1. **P3-01 路由与会话隔离（Implemented）**：独立 `/chat`、`/chat/:conversationId`、`/wiki` 路由与页面组件，按路由加载；以后新增页面也必须使用独立路由和页面组件，不把所有页面代码放进 `App.tsx`。新会话在首条消息完成 metadata 后取得新的 conversationId，历史只恢复所选会话，刷新与项目切换不串会话。
2. **P3-02 历史生命周期与消息（Implemented）**：单会话删除与关联状态安全处置；旧 AI 回复保持可见，流失败和过期回调不抹掉其他轮次。
3. **P3-03 聊天界面（Implemented）**：输入框默认紧凑可展开；确认表单悬浮在聊天内容之上；来源文章列表可收起。
4. **P3-04 自然语言 Tool 与引用（Implemented）**：自然语言产生结构化 Action Intent；Java 继续独占权限、审批和写入。回答引用与实际使用的项目证据关联，不把检索候选冒充引用。
5. **P3-05 语音输入（Implemented）**：实时 ASR 转写进入可编辑输入框，用户主动发送；音频/凭据不进入公开历史或浏览器代码。
6. **P3-06 审批时限（Implemented）**：悬浮确认默认 60 秒倒计时；高危操作到期不得自动通过，自动处理仅用于明确允许的低风险操作且由 Java 复核、幂等和审计。
7. **P3-07 Wiki 链接图谱（Implemented）**：`/wiki/graph` 可视化有来源的 Wiki 页面链接关系，提供浏览和跳转。保留 V3-04 图领域模型、V3-05 抽取和 V3-07 GraphRAG 原计划，不提前实现或宣传这些能力。

其中 P3-01/02/04/06 涉及会话、公共契约或审批，按 L3 风险与实际影响域验证和 Milestone Review；其他阶段按规划器与真实调用关系决定门禁。
