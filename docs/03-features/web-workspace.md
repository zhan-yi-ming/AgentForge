...
AI 文本整理沿用当前项目 SSE Chat，但单次完整 `message`（提示词与原文合计）最多 16,000 字符。Web 在提交前显示输入长度，超限时保留草稿并提示分段整理，不发送必然被拒绝的请求。服务端继续验证同步与流式 Chat，空白消息及超过上限的输入返回 400。聊天会话页的新建会话、语音输入和输入框缩放操作放在发送按钮左侧的一组紧凑图标；录音状态和取消操作保持可访问。登录页在常见桌面与手机视口完整显示品牌、表单和底部说明；极短视口允许滚动以避免裁切登录控件。
