...
首次访问者从品牌介绍与登录卡片组成的响应式双栏入口获知产品价值，以及账号是简历上的邮箱、密码是微信号；页面不显示或内置凭据明文。登录后，Agent Chat 位于单列主内容中央和首位，Wiki、Task、AI 文本整理按纵向次序保留。轻量新手引导说明项目选择、提问、写入确认与下方工具，降低 HR/面试官首次体验成本。
...
5. 响应包含 pending action 时，Web 展示 action 类型、Task、字段和预期 version。确认/拒绝只调用 Java action API，并为该 action 生成稳定 Idempotency Key；网络失败重试复用该 key。只有 `EXECUTED` 成功才刷新 Task，`FAILED` 显示稳定失败信息。
   P3-03 在聊天路由中把待决 Action 放进覆盖内容的确认弹层；Esc 或“稍后处理”仅隐藏界面，可从待决入口重新打开，不能触发业务决策。弹层给出 Action 当前字段，确认/拒绝仍需用户主动操作。
...
## V3 前置页面（P3-01，Implemented）

首页、聊天 `/chat`（及指定会话 `/chat/:conversationId`）、Wiki `/wiki` 是独立路由和页面组件。后续新增页面沿用一页一路由，页面代码按路由加载；共享的认证、项目选择与 typed API 留在应用壳。路由切换取消旧页面在途请求，进入指定聊天 ID 时只显示该会话，未授权或不存在时显示错误状态而不复用其他会话内容。

## P3-03 聊天界面

聊天页输入框首次进入默认紧凑，用户可放大和缩小；引用来源列表默认收起，并在每条回答内独立展开，不因新回答改变旧轮次状态。待决 Action 弹层独立覆盖聊天与来源列表，键盘可关闭并重新打开，关闭不执行 Action。历史删除确认弹窗初始聚焦“取消”，Esc 只关闭弹窗。批准和拒绝由显式按钮调用 Java，现阶段不自动倒计时通过；字段编辑需要后端可验证的公共契约，不在仅前端模拟。
