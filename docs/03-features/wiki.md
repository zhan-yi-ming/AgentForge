...
当前项目的 `/wiki/graph` 是独立按需加载的只读页面。图中每个主节点对应真实 Wiki 页面，连线只来自页面正文中可解析且目标存在的显式 `[[页面标题]]`、`[[页面标题|显示词]]`、`[[Wiki ID]]` 或指向 Wiki ID 的 Markdown 链接；同一来源和目标的多处引用归为一条边并保留显示词。没有链接的页面仍可浏览，失效目标不显示虚构关系。搜索和选中详情可定位页面，用户可打开 Wiki 编辑页；浏览、缩放与拖动不写入数据。项目切换后只使用新项目页面，不混用旧项目内容。该视图不代表语义实体图、自动抽取或 GraphRAG。

P3-07 验收视觉修正：`/wiki/graph` 占满浏览器视口，不嵌在 Wiki 工作台卡片中。星球和轨道只呈现真实页面与显式连接；中心项目装饰节点不表示额外知识实体。侧栏只列真实当前项目页面。
