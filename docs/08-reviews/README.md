...
用户已明确重新授权 Pi 仅执行代码审核；测试仍由 Codex 执行。用户同时持续授权：Codex 在每个阶段完成真实测试和敏感信息扫描后，可把该阶段的源码、配置、测试与文档 diff 发送给外部 DeepSeek Pi V4.1 Flash（`deepseek/deepseek-flash`）。旧 monitor、OnCodexWake、Pi validation 与自动阶段推进保持停用。Pi 需要登录、额度或环境介入时立即请用户处理，不做无限循环重试。
